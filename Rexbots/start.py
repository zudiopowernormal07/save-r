...
async def download_upload_public_with_progress(client, message, username, msgid, target_chat, 
    ...
    # FIRST TRY: Direct copy (fastest method)
    try:
        ...
        logger.debug(f"Copy failed for {msgid}, trying download: {copy_error}")
        pass  # Copy failed, proceed to download

    # SECOND TRY: Download and upload
    temp_client = Client(
        "public_temp",
        api_id=API_ID,
        api_hash=API_HASH,
    ...
    try:
        ...
        logger.error(f"Connection error in public download: {e}")
        try:
...
async def download_upload_private_with_progress(client, message, user_id, msgid, datas, target_chat, 
    ...
    # FIRST TRY: Direct copy using user session (fastest method)
    try:
        acc_copy = Client(
            "saverestricted_copy",
        ...
        await acc_copy.connect()

        chatid = int("-100" + datas[4])

        # Try to copy directly first
        try:
            await acc_copy.copy_message(
                chat_id=target_chat,
                from_chat_id=chatid,
                message_id=msgid
            )
            await acc_copy.disconnect()
            return  # Success - no need to download
        except Exception as copy_error:
            logger.debug(f"Copy failed for private {msgid}, trying download: {copy_error}")
            await acc_copy.disconnect()
            pass  # Copy failed, proceed to download
    except Exception as e:
        logger.debug(f"Copy client error: {e}")
        pass
    ...
    try:
        ...
        logger.error(f"Connection error in private download: {e}")
        try:
