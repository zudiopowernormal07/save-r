...
BATCH_STATE = {}  # User ID -> {step, link, destination, timer}
...
• Auto-start in Chat if no reply in 20 seconds
...
async def batch_start(client: Client, message: Message):
    ...
    # Check if user has dump chat set
    dump_chat_id = await db.get_dump_chat(user_id)

    BATCH_STATE[user_id] = {
        "step": "waiting_destination",
        "link": None,
        "destination": None,  # Will be 'chat' or 'channel'
    ...
    if dump_chat_id:
        ...
        dest_text += f"📢 <b>Channel</b> - Send to channel ({dump_chat_id})\n\n"
    else:
        dest_text += f"<b>⚠️ No Channel Set!</b>\n"
        dest_text += f"Use <code>/setchat &lt;channel_id&gt;</code> to set a channel.\n\n"
        dest_text += f"For now, files will only go to Chat.\n\n"
    ...
    # Start 20-second timer for auto-default
...
async def batch_auto_timer(client, chat_id, msg_id, user_id):
    ...
    # Check if user still waiting and hasn't selected
...
async def batch_destination_callback(client: Client, callback_query: CallbackQuery):
    ...
    if data == "dest_chat":
        ...
        if not dump_chat_id:
            ...
            # Stay on same screen, don't change step
            return
...
async def process_batch_with_destination(client: Client, message: Message):
    """Process batch with user-selected destination"""
    user_id = message.from_user.id
    text = message.text.strip()
    state = BATCH_STATE[user_id]
    destination = state["destination"]  # 'chat' or 'channel'
    ...
    try:
        ...
        # Show destination in progress
        dest_text = "📱 Chat" if destination == "chat" else f"📢 Channel ({dump_chat_id})"
        ...
        for idx, msgid in enumerate(range(start_id, end_id + 1), 1):
            ...
            try:
                if is_public:
                    username = datas[3]
                    try:
                        # Send based on destination choice
                        if destination == "chat":
                            # Only to user
                            await client.copy_message(
                                chat_id=message.chat.id,
                        ...
                        elif destination == "channel" and dump_chat_id:
                            # Only to channel
                            await client.copy_message(
                    ...
                    # Private channel
                    await handle_batch_private_destination(
                        client, message, user_id, msgid, datas, destination, dump_chat_id
                    )
                    success_count += 1
...
async def handle_batch_private_destination(client, message, user_id, msgid, datas, destination, dump_chat_id):
    """Handle private channel batch with specific destination"""
    ...
    try:
        ...
        # Send to selected destination only
        if msg.document:
            await client.send_document(target_chat, msg.document.file_id, caption=msg.caption)
        elif msg.video:
            await client.send_video(target_chat, msg.video.file_id, caption=msg.caption)
        elif msg.photo:
            await client.send_photo(target_chat, msg.photo.file_id, caption=msg.caption)
        elif msg.audio:
            await client.send_audio(target_chat, msg.audio.file_id, caption=msg.caption)
        elif msg.text:
