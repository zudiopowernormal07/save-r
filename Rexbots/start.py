...
async def batch_start(client: Client, message: Message):
    ...
    await message.reply_text(
        ...
        "<i>I'll download all messages in that range.</i>\n\n"
        "❌ Send /cancel to stop",
        parse_mode=enums.ParseMode.HTML
    )
...
async def process_batch_link(client: Client, message: Message):
    """Process batch download from BATCH_STATE"""
    user_id = message.from_user.id
        ...
        for idx, msgid in enumerate(range(start_id, end_id + 1), 1):
            ...
            try:
                if is_public:
                    ...
                    await handle_batch_private(client, message, user_id, msgid, datas)
                    success_count += 1
...
async def handle_batch_private(client, message, user_id, msgid, datas):
    """Handle private channel batch download"""
