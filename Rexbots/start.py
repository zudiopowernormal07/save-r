...
<b>⚜️ Powered By : <a href="https://t.me/Divyanshshukla7">💫『 𝒟𝒾𝓋𝓎𝒶𝓃𝓈𝒽 𝓈𝒽𝓊𝓀𝓁𝒶 』💫😎</a></b>"""
...
# ========== MAIN BATCH PROCESSING WITH ORIGINAL FILENAME ==========
async def process_batch_with_destination(client: Client, message: Message):
    """Process batch with user-selected destination - PRESERVES ORIGINAL FILENAME"""
    user_id = message.from_user.id
...
async def download_upload_public(client, message, username, msgid, target_chat):
    """Download and upload from public channel - PRESERVES ORIGINAL FILENAME"""
    temp_client = Client(
        "public_temp",
        api_id=API_ID,
        api_hash=API_HASH,
    ...
    try:
        ...
        # Handle different media types with ORIGINAL FILENAME
        if msg.document:
            ...
            if msg.video.file_name:
                ...
                if msg.video.mime_type:
                    if "mp4" in msg.video.mime_type:
                        ...
                        ext = ".mkv"
                    elif "avi" in msg.video.mime_type:
                        ext = ".avi"
...
async def download_upload_private(client, message, user_id, msgid, datas, target_chat):
    """Download and upload from private channel - PRESERVES ORIGINAL FILENAME"""
    user_session = await db.get_session(user_id)
    if not user_session:
...
async def handle_restricted_content(client: Client, acc, message: Message, chat_target, msgid):
    ...
    try:
        ...
        if custom_caption:
            final_caption = custom_caption.format(filename=file_path.split("/")[-1], size=humanbytes(file_size))
        else:
            final_caption = script.CAPTION.format(file_name=file_path.split("/")[-1])
            if msg.caption:
                final_caption += f"\n\n{msg.caption}"

        if msg_type == "Document":
            await client.send_document(message.chat.id, file_path, thumb=ph_path, caption=final_caption, progress=progress, progress_args=[message, "up"])
        elif msg_type == "Video":
            await client.send_video(message.chat.id, file_path, duration=msg.video.duration, width=msg.video.width, height=msg.video.height, thumb=ph_path, caption=final_caption, progress=progress, progress_args=[message, "up"])
        elif msg_type == "Audio":
            await client.send_audio(message.chat.id, file_path, thumb=ph_path, caption=final_caption, progress=progress, progress_args=[message, "up"])
        elif msg_type == "Photo":
