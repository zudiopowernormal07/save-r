...
BATCH_STATE = {}  # User ID -> batch state
...
• Bot will download all files in that range.
• <b>NEW:</b> Auto-forward to your dump channel!
...
# ========== BATCH COMMAND WITH DUMP CHAT SUPPORT ==========
...
async def batch_start(client: Client, message: Message):
    ...
    await message.reply_text(
        ...
        "<b>For single message:</b>\n"
        "<code>https://t.me/channel/100</code>\n\n"
        "<i>Files will be sent to you AND your dump channel (if set).</i>\n\n"
        "❌ Send /cancel to stop",
        parse_mode=enums.ParseMode.HTML
    )
...
async def save(client: Client, message: Message):
    ...
    if user_id in BATCH_STATE:
        return await process_batch_link(client, message)
...
async def process_batch_link(client: Client, message: Message):
    """Process batch download with DUMP CHAT support"""
    user_id = message.from_user.id
    text = message.text.strip()

    # GET DUMP CHAT (CHANNEL) FOR THIS USER
    ...
    try:
        ...
        # Show dump chat info in progress
        dump_info = f"\n<b>📤 Forward to:</b> <code>{dump_chat_id}</code>" if dump_chat_id else ""
            ...
            try:
                if is_public:
                    ...
                    try:
                        ...
                        # 2. ALSO SEND TO DUMP CHAT (CHANNEL) IF SET
                        if dump_chat_id:
                            try:
                                await client.copy_message(
                                ...
                                logger.info(f"Forwarded message {msgid} to dump chat {dump_chat_id}")
                            except Exception as e:
                        ...
                        logger.error(f"Public copy failed for {msgid}: {e}")
                        failed_count += 1
                else:
                    # Private channel with dump chat support
                    await handle_batch_private_with_dump(
                        client, message, user_id, msgid, datas, dump_chat_id
                    )
                    success_count += 1
        ...
        # Final summary with dump chat info
        dump_summary = f"\n<b>📤 Forwarded to:</b> <code>{dump_chat_id}</code>" if dump_chat_id else ""
...
async def handle_batch_private_with_dump(client, message, user_id, msgid, datas, dump_chat_id):
    """Handle private channel batch download with DUMP CHAT support"""
    ...
    try:
        ...
        # Send to user first, then to dump chat
        if msg.document:
            await client.send_document(message.chat.id, msg.document.file_id, caption=msg.caption)
            if dump_chat_id:
                await client.send_document(dump_chat_id, msg.document.file_id, caption=msg.caption)

        elif msg.video:
            await client.send_video(message.chat.id, msg.video.file_id, caption=msg.caption)
            if dump_chat_id:
                await client.send_video(dump_chat_id, msg.video.file_id, caption=msg.caption)

        elif msg.photo:
            await client.send_photo(message.chat.id, msg.photo.file_id, caption=msg.caption)
            if dump_chat_id:
                await client.send_photo(dump_chat_id, msg.photo.file_id, caption=msg.caption)

        elif msg.audio:
            await client.send_audio(message.chat.id, msg.audio.file_id, caption=msg.caption)
            if dump_chat_id:
                await client.send_audio(dump_chat_id, msg.audio.file_id, caption=msg.caption)

        elif msg.text:
            await client.send_message(message.chat.id, msg.text)
            if dump_chat_id:
                await client.send_message(dump_chat_id, msg.text)
